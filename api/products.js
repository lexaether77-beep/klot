export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const savedAt  = (stored && stored.savedAt)  || 0;
      if (!products) return res.status(200).json({ products: null });
      return res.status(200).json({ products, version, savedAt });
    } catch (e) {
  ...
  if (req.method === 'POST') {
    ...
    const { products, version, savedAt } = req.body || {};
    ...
    try {
      await fetch(`${redisUrl}/pipeline`, {
        ...
        body: JSON.stringify([
          ['SET', 'klot:products', JSON.stringify({ products, version: version || null, savedAt: savedAt || 0 })],
        ]),
      });
