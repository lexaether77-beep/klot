...
export default function handler(req, res) {
  ...
  // Authenticated — serve the admin panel
  try {
    const filePath = path.join(process.cwd(), 'api', 'admin-page.html');
    ...
    return res.status(500).send('<h1>Admin panel not found</h1><p>admin-page.html is missing from /api/</p>');
  }
}
...
<div class="box">
  ...
  <div id="err" style="display:none;background:rgba(248,113,113,.1);border:1px solid rgba(248,113,113,.3);color:#fca5a5;font-size:12px;padding:10px 14px;margin-bottom:20px;text-align:center"></div>
  <form id="loginForm">
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    if (res.status === 302 || res.type === 'opaqueredirect') {
      ...
      // Extract error message if login page returned
      var match = text.match(/class="err"[^>]*>([^<]+)/);
      err.textContent = match ? match[1] : 'Incorrect password.';
